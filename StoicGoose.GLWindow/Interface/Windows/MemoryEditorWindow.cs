...
{
    public class MemoryEditorWindow : WindowBase
    {
        ...
        public MemoryEditorWindow() : base("Memory Editor")
        {
        ...
        {
            if (userData is not (IMachine, bool)) return;
        ...
        {
            if (userData is not (IMachine machine, bool isRunning)) return;
            ...
            if (ImGui.Begin(WindowTitle, ref isWindowOpen, ImGuiWindowFlags.NoScrollbar))
            {
                ...
                DrawContents(machine, isRunning);
        ...
        private void DrawContents(IMachine machine, bool isRunning)
        {
            ...
            if (ShowDataPreview)
                footerHeight += heightSeparator + ImGui.GetFrameHeightWithSpacing() + (ImGui.GetTextLineHeightWithSpacing() * (PreviewShiftJis ? 4.5f : 3.0f));
            ...
            ImGui.BeginChild("##scrolling", new NumericsVector2(0f, -footerHeight), false, ImGuiWindowFlags.NoMove | ImGuiWindowFlags.NoNav | ImGuiWindowFlags.NoScrollWithMouse);
            {
                ...
                clipper.Begin(lineCountTotal, lineHeight);
                {
                    ...
                    var previewDataTypeSize = ShowDataPreview && dataTypeSizes.TryGetValue(previewDataType, out int value) ? value : 0;
                        ...
                        for (var i = clipper.DisplayStart; i < clipper.DisplayEnd; i++)
                        {
                            ...
                            if (ShowAscii)
                            {
                                ...
                                for (var j = 0; j < NumColumns && addr < memorySize; j++, addr++)
                                {
                                    ...
                                    drawList.AddText(pos, (displayCh == ch) ? colorText : colorDisabled, new string([displayCh]));
                                    pos.X += glyphWidth;
                                }
                            }
                        }
        ...
        private void DrawPreviewLine(IMachine machine, uint colorText, uint colorDisabled)
        {
            ...
            if (ImGui.BeginCombo("##combo_type", dataTypeDescs.TryGetValue(previewDataType, out string value) ? value : "N/A", ImGuiComboFlags.HeightLargest))
            {
                foreach (ImGuiDataType type in Enum.GetValues<ImGuiDataType>())
                    if (dataTypeDescs.TryGetValue(type, out string value1) && ImGui.Selectable(value1, previewDataType == type))
                        previewDataType = type;
                ImGui.EndCombo();
            }
            ImGui.SameLine();
            ImGui.SetNextItemWidth((glyphWidth * 6f) + style.FramePadding.X * 2f + style.ItemInnerSpacing.X);
            ImGui.Combo("##combo_endianness", ref previewEndianness, ["LE", "BE"], 2);
            ...
            foreach (DataFormat format in Enum.GetValues<DataFormat>())
            {
                ...
                if (hasValue && format == DataFormat.Bin)
                {
                    for (var i = 0; i < buf.Length; i++)
                        drawList.AddText(new NumericsVector2(pos.X + dist + (i * (glyphWidth - 1f)), pos.Y),
                            grayOutZeroes && buf[i] == '0' ? colorDisabled : colorText, new string([buf[i]]));
                }
                else
        ...
        {
            if (!dataTypeSizes.TryGetValue(dataType, out int elemSize)) return;
        ...
        private static void DrawPreviewSjis(IMachine machine, int addr, ref string outBuf)
        {
