...
namespace StoicGoose.ImGuiCommon.Handlers
{
    public class FileDialogHandler
    {
        ...
        readonly List<(string label, string[] filter)> filters = [];
        int selectedFilter = -1;

        DirectoryInfo workingDirectory = default;
        List<DirectoryInfo> currentSubDirectories = [];
        List<FileInfo> currentFiles = [];

        readonly List<(string label, bool isDirectory, int index)> currentFileDirList = [];
        ...
        public void Draw()
        {
            ...
            for (var i = 0; i < fileDialogs.Count; i++)
            {
                ...
                if (driveInfos == default)
                {
                    ...
                    driveLabels = [.. driveInfos.Select(x => $"{x.Name} {(x.IsReady ? $"[{x.VolumeLabel}]" : string.Empty)}")];
                }
        ...
        private void UpdateCurrentFilesAndDirs(DirectoryInfo directory)
        {
            ...
            {
                currentFiles = [.. workingDirectory.GetFiles().Where(x => filters[selectedFilter].filter.Contains(x.Extension.ToLower()))];
                currentSubDirectories = [.. workingDirectory.GetDirectories()];
    ...
    public partial class FileDialog(ImGuiFileDialogType dialogType, string title = "")
    {
        public ImGuiFileDialogType DialogType { get; set; } = dialogType;
        ...
        public string[] GetFilterExtensions(int index) => [.. GetFilterExtensionsRegex().Matches(Filter)[index].Value.Split('|')[1].Split(';').Select(x => x[x.LastIndexOf('.')..])];

        [GeneratedRegex(@"[^|]+\|[^|]+")]
        private static partial Regex GetFilterExtensionsRegex();
    }
}
